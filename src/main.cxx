...
char *make_json_string(codegen_response_t* response,std::string jsonname ) {
...
#endif
    sprintf(output,"{\"code\":\"%s\"}",response->codegen->getCodeString().c_str());
    fout<<response->codegen->getCodeString().c_str()<<endl;
    fout.close();
    return output;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        for(int i=0;i<count;i++) {
            ...
            char *output = make_json_string(response,argv[2]);
        ...
        while(done<count) {
            ...
            for(int i=0;i<num_threads;i++) {
                if (parm[i]->done) {
                    ...
                    char *json = make_json_string(response,argv[2]);
